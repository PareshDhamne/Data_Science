# -*- coding: utf-8 -*-
"""
Created on Wed May 31 00:25:32 2023

@author: Hp
"""

import pandas as pd
df=pd.read_csv('D:/PARESH1_python/Computer_Data.csv')
df
...
df=pd.read_csv('D:/PARESH1_python/Computer_Data.csv')
df15=df.astype({"price":str,"speed":int})
print(df15.dtypes)
...
#ignores error
df16=df.astype({"price":int},errors='ignore')
df16.dtypes

#Generates error
df17=df.astype({"price":float},errors='raise')
df17.dtypes
###############################################
###############################################
#Drop column by name
df18=df.drop(['price'],axis=1)
print(df18)

#Explictily using parameter name 'labels'
df19=df.drop(labels=["price"],axis=1)
df19
...
def add_4(x):
    return x+4
df49=df["speed"].apply(add_4)
df49

#apply to multiple columns
def add_10(x):
    return x+10
df50=df[['speed','price']].apply(add_10)
df50

#apply lambda fun to single column
df51=df["speed"].apply(lambda x:x-2)
df51

#using DataFrame.transform()
def add_2(x):
    return x+2
df52=df['speed'].transform(add_2)
df52

#using pandas>dataframe.map() to single column

df52=df['speed'].map(lambda Sales: Sales/2.)
df52
