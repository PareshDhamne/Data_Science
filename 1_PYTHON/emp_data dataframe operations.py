# -*- coding: utf-8 -*-
"""
Created on Wed May 31 00:02:22 2023
...
df=pd.read_csv("D:\PARESH1_python\emp_data.csv")
df15=df.astype({"Salary_hike":str,"Churn_out_rate":int})
print(df15.dtypes)
...
#ignores error
df16=df.astype({"Churn_out_rate":int},errors='ignore')
df16.dtypes

#Generates error
df17=df.astype({"Churn_out_rate":float},errors='raise')
df17.dtypes
###############################################
###############################################
#Drop column by name
df18=df.drop(['Churn_out_rate'],axis=1)
print(df18)

#Explictily using parameter name 'labels'
df19=df.drop(labels=["Churn_out_rate"],axis=1)
df19

#Alternativly you can also use columns insted of labels
df20=df.drop(columns=["Churn_out_rate"],axis=1)
df20
...
df=pd.read_csv("D:\PARESH1_python\emp_data.csv")
df21=df.drop(["Salary_hike","Churn_out_rate"],axis=1)
df21
...
df37=df.loc[:,["Salary_hike","Churn_out_rate"]]
df37


#select columns between two cloumns
df39=df.loc[:,'Salary_hike':'Churn_out_rate']
df39

#select column by range
df40=df.loc[:,'Churn_out_rate':]
df40
...
df42=df.query("Churn_out_rate=='4.15'")
df42
###########################################

#not equals condition
df43=df.query("Churn_out_rate != '25'")
df43
####################################
df=pd.read_csv("D:\PARESH1_python\emp_data.csv")
df
...
df44=df.rename(columns={'Churn_out_rate':'rate'})
df44.columns

#for multiple columns
df=pd.read_csv("D:\PARESH1_python\emp_data.csv")
df45=df.rename(columns={'Salary_hike':'Bonus','Churn_out_rate':'rate'})
df45

#axis=1 or axis='columns'
df46=df.rename({'Churn_out_rate':'rate'},axis=1)
df46
df47=df.rename({'Churn_out_rate':'rate'},axis='columns')
df47
...
def add_4(x):
    return x+4
df49=df["Churn_out_rate"].apply(add_4)
df49

#apply to multiple columns
def add_10(x):
    return x+10
df50=df[['Churn_out_rate','Salary_hike']].apply(add_10)
df50

#apply lambda fun to single column
df51=df["Salary_hike"].apply(lambda x:x-2)
df51

#using DataFrame.transform()
def add_2(x):
    return x+2
df52=df['Salary_hike'].transform(add_2)
df52

#using pandas>dataframe.map() to single column

df52=df['Salary_hike'].map(lambda Sales: Sales/2.)
df52

df
def add_4(x):
    return x+4
df49=df["Salary_hike"].apply(add_4)
df49
...
import numpy as np
df53=df['Salary_hike'].apply(np.square)
df53
#######################################
#using Numpy.square() method
#using numpy.square() and [] operator
df54=np.square(df['Salary_hike'])
df54

#groupby() function
#for single column
df=pd.read_csv("D:\PARESH1_python\emp_data.csv")
df55=df.groupby(['Salary_hike']).sum()
df55
##########################################3

#group by multiple columns
df56=df.groupby(['Salary_hike','Churn_out_rate']).sum()
df56
######################################
#add index to the groupby data
#add row index to the group by result
df57=df.groupby(['Salary_hike','Churn_out_rate']).sum().reset_index()
df57
################################
